...
```bash
mkvirtualenv cdt_accounting -p python3.12
...
Create a database named `cdt_accounting`.

```
createdb cdt_accounting
```
...
```bash
celery -A cdt_accounting worker -l INFO --pool=solo
```

Or with celery beat (for scheduled tasks):

```bash
celery -A cdt_accounting worker -l INFO -B --pool=solo
```
